...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    // Render
    const ImU32 col = (held && hovered) ? ImColor(111, 155, 252, 100) : hovered ? ImColor(111, 155, 252, 140) : ImColor(111, 155, 252, 190);
    RenderNavHighlight(bb, id);
    
    if (!(flags & ImGuiButtonFlags_Transparent))
    {
        window->DrawList->AddRectFilled(bb.Min, bb.Max, col, 5.f);
        if (hovered)
            window->DrawList->AddRect(bb.Min, bb.Max, ImColor(111, 155, 252, 100), 5.f);
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const ImVec2 pos = window->DC.CursorPos;
    const ImRect total_bb(pos, pos + ImVec2(square_sz + (label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f), label_size.y + style.FramePadding.y * 2.0f) + ImVec2(7, 0));
    ItemSize(total_bb, style.FramePadding.y);
    ...
    ImU32 col = (held && hovered) ? ImColor(111, 155, 252, 60) : hovered ? ImColor(111, 155, 252, 45) : ImColor(0, 14, 28, 210);
    //RenderFrame(check_bb.Min, check_bb.Max, GetColorU32((held && hovered) ? ImGuiCol_FrameBgActive : hovered ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg), true, style.FrameRounding);
    window->DrawList->AddRectFilled(check_bb.Min + ImVec2(0, 5), check_bb.Max - ImVec2(0, 5), col, 7.f);
    if (*v)
    {
        window->DrawList->AddRectFilled(check_bb.Max - ImVec2(12, 17), check_bb.Max - ImVec2(2, 7), ImColor(111, 155, 252, 220), 7.f);
    }
    else
    {
        window->DrawList->AddRectFilled(check_bb.Min + ImVec2(2, 7), check_bb.Min + ImVec2(12, 17), ImColor(111, 155, 252, 100), 7.f);
    }
